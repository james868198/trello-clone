    ...
    reducers: {
        ...
        updateListTitle: (state, action) => {
            ...
            if (
                ...
                title == null ||
                title === "" ||
                title === state.lists[listId].title
            )
                return
        ...
        updateListBoardId: (state, action) => {
            ...
            if (
                ...
                boardId == null ||
                boardId === "" ||
                boardId === state.lists[listId].boardId
            )
                return
        ...
        removeListById: (state, action) => {
            ...
            if (!state.lists.hasOwnProperty(listId)) return
            if (soft) state.lists[listId].archived = true
            else delete state.lists[listId]
        },
        removeListByBoardId: (state, action) => {
            ...
            for (const [id, list] of Object.entries(state.lists)) {
                if (boardId !== list.boardId) continue
                if (soft) state.lists[id].archived = true
                else delete state.lists[id]
            }
        },
        addCardToList: (state, action) => {
            const { listId, cardId } = action.payload
            const list = state.lists[listId]
            if (list && cardId) list.cards.push(cardId)
        },
        swapCardInBoard: (state, action) => {
            const { listId, targetCardId, enterCardId, over } = action.payload
            if (listId == null || targetCardId == null || enterCardId == null) return
            ...
            if (targetIndex === enterIndex || enterIndex >= list.cards.length) return
        ...
        insertCardToList: (state, action) => {
            ...
            if (targetList == null || enterList == null) return
            ...
            if (enterIndex < enterList.cards.length) enterList.cards.splice(enterIndex, 0, targetCardId)
            else enterList.cards.push(targetCardId)
        },
        removeCardFromList: (state, action) => {
            const { listId, cardId } = action.payload
            const list = state.lists[listId]
            if (list && cardId) list.cards = list.cards.filter((id) => id !== cardId)
        },
    },
})
...
export const {
    ...
    removeCard,
    removeCardFromList,
} = trelloListSlice.actions

export const getListById = (listId) => (state) =>
    state.trelloList.lists.hasOwnProperty(listId) ? state.trelloList.lists[listId] : null
