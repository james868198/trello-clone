...
export const trelloBoardsSlice = createSlice({
    ...
    reducers: {
        ...
        updateBoardName: (state, action) => {
            ...
            if (state.boards[boardId] && state.boards[boardId].name !== name) state.boards[boardId].name = name
        },
        ...
        starBoard: (state, action) => {
            ...
            if (state.boards[boardId]) state.boards[boardId].starred = star
        },
        archiveBoard: (state, action) => {
            const { boardId } = action.payload
            if (state.boards[boardId]) state.boards[boardId].archived = true
        },
        removeBoard: (state, action) => {
            const { boardId } = action.payload
            if (state.boards[boardId]) delete state.boards[boardId]
        },
        addListToBoard: (state, action) => {
            const { boardId, listId } = action.payload
            const board = state.boards[boardId]
            if (board && listId) board.lists.push(listId)
        },
        swapListInBoard: (state, action) => {
            const { boardId, targetListId, enterListId, over } = action.payload
            if (boardId == null || targetListId == null || enterListId == null) return
            ...
            if (targetIndex === enterIndex || enterIndex >= board.lists.length) return
        ...
        removeListFromBoard: (state, action) => {
            const { boardId, listId } = action.payload
            const board = state.boards[boardId]
            if (board && listId) board.lists = board.lists.filter((listData) => listData !== listId)
        },
    },
})
...
export const {
    ...
    insertListToBoard,
    removeListFromBoard,
} = trelloBoardsSlice.actions

// select board
export const getBoards = (state) => Object.values(state.trelloBoard.boards)
export const getBoardById = (boardId) => (state) =>
    state.trelloBoard.boards.hasOwnProperty(boardId) ? state.trelloBoard.boards[boardId] : null
