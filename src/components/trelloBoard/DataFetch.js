...
import {
    ...
    addCardToList,
    swapCardInBoard,
} from "../../store/slice/trelloListSlice"
...
import {
    ...
    updateChecklist,
    removeChecklist,
} from "../../store/slice/trelloCardSlice"
...
function swapList(targetListId, enterListId, over) {
    ...
    if (targetList == null || enterList == null || targetList.boardId !== enterList.boardId) return
    store.dispatch(
        swapListInBoard({
...
function updateCardOrder(targetCardId, enterCardId, over) {
    ...
    if (targetCard == null || enterCard == null || targetCardId === enterCardId) return
...
function AddChecklist(name, cardId) {
    if (name == null || name === "" || cardId == null) return
...
function UpdateChecklist(name, cardId, index) {
    if (name == null || name === "" || cardId == null || index == null) return
    store.dispatch(
        updateChecklist({
            cardId: cardId,
            index: index,
...
function RemoveChecklist(cardId, index) {
    if (cardId == null || index == null) return
    store.dispatch(
        removeChecklist({
            cardId: cardId,
            index: index,
