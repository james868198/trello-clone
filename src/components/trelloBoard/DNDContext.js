...
export default function DNDProvider(props) {
    ...
    const handleOnDragStart = (e) => {
        ...
        if (type === "card") {
            if (draggedCard && e.target.id === draggedCard.current) return
            draggedCard.current = e.target.id
            draggedList.current = null
        } else if (type === "list") {
            if (draggedCard && e.target.id === draggedList.current) return
    ...
    const handleOnDragEnd = (e) => {
        e.stopPropagation()
        if (draggedCard) draggedCard.current = null
        if (draggedList) draggedList.current = null
    }
