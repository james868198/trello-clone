...
export default function TextModal({ cardId, open, handleCloseModal }) {
    ...
    useEffect(() => {
        if (card && titleRef && titleRef.current) titleRef.current.value = card.title
    }, [])
    ...
    return (
        <Modal open={open}>
            ...
            <ModalContent>
                <ModalBody>
                    <TextField ref={titleRef} onClick={(event) => event.stopPropagation()} />
                    <SaveButton>
                        <Button variant="contained" onClick={(event) => handleSaveChange(event)}>
                            Save Change
                        </Button>
                    </SaveButton>
                </ModalBody>
